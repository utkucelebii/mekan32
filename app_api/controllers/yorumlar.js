 ...
 if(!req.params.mekanid || !req.params.yorumid){
	...
	.exec(
		function(hata,gelenMekan){
			var yorum;
			if(!gelenMekan){
				cevapOlustur(res, 404, {"mesaj":"mekanid ublunamadı"});
				return;
			}else if(hata){
				cevapOlustur(res, 400, hata);
				return;
...
const yorumSil=function(req, res) {
 if(!req.params.mekanid || !req.params.yorumid){
	...
	.exec(
		function(hata,gelenMekan){
			if(!gelenMekan){
				cevapOlustur(res, 400, {"mesaj":"mekanid bulunamadı"});
				return;
			}else if(hata){
				cevapOlustur(res, 400, hata);
				return;
				...
				if(gelenMekan.yorumlar && gelenMekan.yorumlar.length > 0){
					if(!gelenMekan.yorumlar.id(req.params.yorumid)){
						...
						gelenMekan.save(function(hata,mean){
							if(hata){
								cevapOlustur(res, 404, hata);
							}else{
								ortalamaPuanGuncelle(gelenMekan._id);
